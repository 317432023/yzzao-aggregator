...
public class ReceiverTask implements Runnable {
	...
	public ReceiverTask(BlockingQueue<JSONObject> storage) {
	...
	private void recv() {
        ...
        logger.debug("Recv Packet=> " + packetToString);
			...
			for( byte[] decode: packlist ) { 
				...
				// 数据放进一个队列
				final boolean b = storage.offer(jsonObj);
				
				if(!b){
				  final String logMsg = new StringBuffer(1500)
            .append("消息队列v1已满[").append(storage.size()).append("]，忽略数据包:").append( "\r\n" )
            .append(StringUtil.bytesToHex(decode)).append( "\r\n" )
    ...
    obj.element("TeamNo",String.format("%d%d%d%d", c_int(fd[50]),c_int(fd[51]),c_int(fd[52]),c_int(fd[53]))); //班别刷卡序列号
    obj.element("menuNo",String.format("%d%d%d%d", c_int(fd[54]),c_int(fd[55]),c_int(fd[56]),c_int(fd[57]))); //菜单设定刷卡序列号
    obj.element("repair_sn",String.format("%d%d%d%d", c_int(fd[58]),c_int(fd[59]),c_int(fd[60]),c_int(fd[61]))); //机修刷卡序列号
